...
void options::LoadSettingsFromFile()
{
#if defined(PSP)
	// On PSP, settings file path
	std::string settingsPath = "ms0:/PSP/GAME/SpaceCadetPinball/settings.ini";
#elif defined(_WIN32)
...
void options::SaveSettingsToFile()
{
#if defined(PSP)
	std::string settingsPath = "ms0:/PSP/GAME/SpaceCadetPinball/settings.ini";
#elif defined(_WIN32)
	...
	// Ensure all options are saved to the settings map
	for (const auto opt : AllOptions)
		opt->Save();
